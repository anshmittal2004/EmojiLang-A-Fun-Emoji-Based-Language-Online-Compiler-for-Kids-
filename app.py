...
class Interpreter:
    ...
    def interpret(self, node: ASTNode):
        if isinstance(node, Program):
            ...
            value = self.evaluate(node.expr)
            yield ("output", str(value))
        elif isinstance(node, InputStmt):
            input_value = yield ("need_input", node.var)
            try:
...
def start_execution():
    ...
    try:
        ...
        gen = interpreter.interpret(ast)
        session_id = str(uuid.uuid4())
        sessions[session_id] = (gen, [])  # Store generator and output list
        output = []
        ...
        try:
            ...
            while True:
                if result[0] == "output":
                    ...
                    result = next(gen)
                elif result[0] == "need_input":
                    sessions[session_id] = (gen, output)
                    return jsonify({
...
def provide_input():
    ...
    gen, output = sessions[session_id]
    try:
        result = gen.send(input_value)
        while True:
            if result[0] == "output":
                output.append(result[1])
                result = next(gen)
